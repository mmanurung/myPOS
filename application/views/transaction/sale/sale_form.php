...
$(document).on('click', '#edit_cart', function() {
    ...
    if (price == '' || price < 1) {
        alert('Harga tdk boleh kosong')
        $('#price_item').focus()
    } else if (qty == '' || qty < 1) {
        alert('Qty tdk boleh kosong')
        $('#qty_item').focus()
    } else {
        $.ajax({
            type: 'POST',
            ...
            success: function(result) {
                if (result.success == true) {
                    ...
                    alert('Data item cart tdk terupdate');
                }
            }
        })
    }
...
$(document).on('click', '#process_payment', function() {
    ...
    if (subtotal < 1) {
        ...
        alert('Cash belum diinput')
        $('cash').focus()
    } else {
        if (confirm('Apa yakin?')) {
            $.ajax({
                type: 'POST',
                url: '<?=site_url('sale/process')?>',
